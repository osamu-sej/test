...
# 実ブラウザ(Chrome)と同等のヘッダーセット。WAF が UA だけでなく
# Sec-Fetch-* / sec-ch-ua の欠落でボット判定するサイト(PPIH 等)への対策
REQUEST_HEADERS = {
    ...
    def fetch_news(self, company_ids, start_date_str, end_date_str):
        ...
        for cid in company_ids:
            ...
            if found_count == 0:
                ...
                # 1つも無ければ、一覧を JavaScript で描画するサイトの可能性が高い
                date_hits = len(DATE_FLEX_FINDALL_RE.findall(
                    unicodedata.normalize("NFKC", html_content or "")
                ))
                debug_logs.append(
                    f"Result: 0 items found. (HTML {len(html_content or '')} chars / 日付らしき文字列 {date_hits} 個)"
                )
                if date_hits == 0:
                    debug_logs.append("  ※ HTML 内に日付が見つかりません → JavaScript 描画サイトの可能性")
